import React, { useState, useEffect } from 'react';
import {
  Table, Button, Form, InputGroup, Pagination, Dropdown, Breadcrumb, Modal
} from 'react-bootstrap';
import { FaPlus, FaQrcode } from 'react-icons/fa';
import axios from '../../axiosConfig';

const SizeTable = () => {
  const [sizesData, setSizesData] = useState({
    content: [],
    totalPages: 0,
    number: 0,
    first: true,
    last: true
  });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  const [showAddModal, setShowAddModal] = useState(false);
  const [showEditModal, setShowEditModal] = useState(false);

  const [newSize, setNewSize] = useState({
    ma: '',
    tenKichCo: '',
    trangThai: 1,
  });

  const [editingSize, setEditingSize] = useState(null);

  const [queryParams, setQueryParams] = useState({
    page: 0,
    size: 5,
    tenKichCo: '',
    trangThai: 'Tất cả'
  });

  const fetchSizes = async () => {
    setLoading(true);
    setError(null);
    try {
      const params = { ...queryParams };
      if (params.trangThai === 'Tất cả') {
        delete params.trangThai;
      }
      const response = await axios.get('/kichThuoc', { params });
      setSizesData(response.data);
    } catch (error) {
      console.error('Lỗi khi lấy dữ liệu:', error);
      setError('Không thể tải dữ liệu. Vui lòng thử lại sau.');
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchSizes();
    // eslint-disable-next-line
  }, [queryParams]);

  const handleAdd = async (e) => {
    e.preventDefault();
    if (!newSize.ma.trim() || !newSize.tenKichCo.trim()) {
      alert('Vui lòng nhập đủ mã và tên kích thước');
      return;
    }
    try {
      await axios.post('/kichThuoc', newSize);
      setShowAddModal(false);
      setNewSize({
        ma: '',
        tenKichCo: '',
        trangThai: 1
      });
      fetchSizes();
    } catch (error) {
      console.error('Lỗi khi thêm:', error);
      alert('Có lỗi xảy ra khi thêm kích thước!');
    }
  };

  const handleEditClick = (size) => {
    setEditingSize({
      id: size.id,
      ma: size.ma,
      tenKichCo: size.tenKichCo,
      trangThai: size.trangThai
    });
    setShowEditModal(true);
  };

  const handleUpdate = async (e) => {
    e.preventDefault();
    if (!editingSize?.ma?.trim() || !editingSize?.tenKichCo?.trim()) {
      alert('Vui lòng nhập đủ mã và tên kích thước');
      return;
    }
    try {
      await axios.put(`/kichThuoc/${editingSize.id}`, {
        ma: editingSize.ma,
        tenKichCo: editingSize.tenKichCo,
        trangThai: editingSize.trangThai
      });
      setShowEditModal(false);
      setEditingSize(null);
      fetchSizes();
    } catch (error) {
      console.error('Lỗi khi cập nhật:', error);
      alert('Có lỗi xảy ra khi cập nhật kích thước!');
    }
  };

  const handleDelete = async (id) => {
    if (window.confirm('Bạn có chắc chắn muốn xóa kích thước này?')) {
      try {
        await axios.delete(`/kichThuoc/${id}`);
        fetchSizes();
      } catch (error) {
        console.error('Lỗi khi xóa:', error);
        alert('Có lỗi xảy ra khi xóa kích thước!');
      }
    }
  };

  const getTrangThaiText = (trangThai) =>
      trangThai === 1 || trangThai === 'Hiển thị' ? 'Hiển thị' : 'Ẩn';

  return (
      <div style={{ background: '#f5f6fa', minHeight: '100vh', padding: '24px', userSelect: 'none' }}>
        <Breadcrumb>
          <Breadcrumb.Item href="/">Trang Chủ</Breadcrumb.Item>
          <Breadcrumb.Item active>Kích Thước</Breadcrumb.Item>
        </Breadcrumb>

        <div className="bg-white p-3 rounded shadow-sm">
          {/* Top actions */}
          <div className="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
            {/* Search */}
            <InputGroup style={{ maxWidth: 300 }}>
              <Form.Control
                  placeholder="Tìm kích thước"
                  value={queryParams.tenKichCo}
                  onChange={(e) =>
                      setQueryParams({
                        ...queryParams,
                        tenKichCo: e.target.value,
                        page: 0
                      })
                  }
              />
            </InputGroup>

            {/* Status filter */}
            <Form.Select
                style={{ maxWidth: 200 }}
                value={queryParams.trangThai}
                onChange={(e) =>
                    setQueryParams({
                      ...queryParams,
                      trangThai: e.target.value,
                      page: 0
                    })
                }
            >
              <option value="Tất cả">Tất cả trạng thái</option>
              <option value={1}>Hiển thị</option>
              <option value={0}>Ẩn</option>
            </Form.Select>

            {/* Action buttons */}
            <div className="d-flex flex-wrap align-items-center gap-2">
              <Dropdown>
                <Dropdown.Toggle variant="outline-secondary" id="dropdown-basic">
                  ☰
                </Dropdown.Toggle>
                <Dropdown.Menu>
                  <Dropdown.Item href="#scan">
                    <FaQrcode className="me-2" />
                    Quét mã
                  </Dropdown.Item>
                  <Dropdown.Item href="#export">📥 Export Excel</Dropdown.Item>
                </Dropdown.Menu>
              </Dropdown>

              <Button variant="primary" onClick={() => setShowAddModal(true)}>
                <FaPlus className="me-1" />
                Thêm kích thước
              </Button>
            </div>
          </div>

          {/* Table */}
          {error ? (
              <div className="alert alert-danger">{error}</div>
          ) : (
              <Table hover responsive>
                <thead>
                <tr>
                  <th>STT</th>
                  <th>Mã</th>
                  <th>Tên</th>
                  <th>Trạng thái</th>
                  <th>Thao tác</th>
                </tr>
                </thead>
                <tbody>
                {loading ? (
                    <tr>
                      <td colSpan="5" className="text-center">
                        <div className="spinner-border" role="status">
                          <span className="visually-hidden">Đang tải...</span>
                        </div>
                      </td>
                    </tr>
                ) : sizesData.content?.length > 0 ? (
                    sizesData.content.map((size, index) => (
                        <tr key={size.id}>
                          <td>{queryParams.page * queryParams.size + index + 1}</td>
                          <td>{size.ma || ''}</td>
                          <td>{size.tenKichCo}</td>
                          <td>
                      <span className={`badge ${getTrangThaiText(size.trangThai) === 'Hiển thị' ? 'bg-success' : 'bg-secondary'}`}>
                        {getTrangThaiText(size.trangThai)}
                      </span>
                          </td>
                          <td className="text-center">
                            <Dropdown align="end">
                              <Dropdown.Toggle variant="light" className="rounded-circle border-0">
                                ⋮
                              </Dropdown.Toggle>
                              <Dropdown.Menu>
                                <Dropdown.Item onClick={() => handleEditClick(size)}>
                                  Sửa
                                </Dropdown.Item>
                                <Dropdown.Item onClick={() => handleDelete(size.id)}>
                                  Xóa
                                </Dropdown.Item>
                              </Dropdown.Menu>
                            </Dropdown>
                          </td>
                        </tr>
                    ))
                ) : (
                    <tr>
                      <td colSpan="5" className="text-center">Không có dữ liệu</td>
                    </tr>
                )}
                </tbody>
              </Table>
          )}

          {/* Pagination + View control */}
          {!loading && sizesData.content?.length > 0 && (
              <div className="d-flex justify-content-between align-items-center mt-3">
                <Form.Select
                    size="sm"
                    style={{ maxWidth: 120 }}
                    value={queryParams.size}
                    onChange={(e) =>
                        setQueryParams({
                          ...queryParams,
                          size: Number(e.target.value),
                          page: 0
                        })
                    }
                >
                  <option value={5}>Xem 5</option>
                  <option value={10}>Xem 10</option>
                  <option value={20}>Xem 20</option>
                </Form.Select>

                <Pagination>
                  <Pagination.Prev
                      disabled={sizesData.first}
                      onClick={() =>
                          setQueryParams({
                            ...queryParams,
                            page: queryParams.page - 1
                          })
                      }
                  />
                  {[...Array(sizesData.totalPages)].map((_, idx) => (
                      <Pagination.Item
                          key={idx}
                          active={idx === sizesData.number}
                          onClick={() =>
                              setQueryParams({
                                ...queryParams,
                                page: idx
                              })
                          }
                      >
                        {idx + 1}
                      </Pagination.Item>
                  ))}
                  <Pagination.Next
                      disabled={sizesData.last}
                      onClick={() =>
                          setQueryParams({
                            ...queryParams,
                            page: queryParams.page + 1
                          })
                      }
                  />
                </Pagination>
              </div>
          )}
        </div>

        {/* Modal Thêm mới */}
        <Modal show={showAddModal} onHide={() => setShowAddModal(false)} centered>
          <Modal.Header closeButton>
            <Modal.Title>Thêm kích thước mới</Modal.Title>
          </Modal.Header>
          <Form onSubmit={handleAdd}>
            <Modal.Body>
              <Form.Group className="mb-3" controlId="formMa">
                <Form.Label>Mã kích thước</Form.Label>
                <Form.Control
                    required
                    type="text"
                    value={newSize.ma}
                    onChange={(e) =>
                        setNewSize({
                          ...newSize,
                          ma: e.target.value
                        })
                    }
                    autoFocus
                />
              </Form.Group>
              <Form.Group className="mb-3" controlId="formTenKichCo">
                <Form.Label>Tên kích thước</Form.Label>
                <Form.Control
                    required
                    type="text"
                    value={newSize.tenKichCo}
                    onChange={(e) =>
                        setNewSize({
                          ...newSize,
                          tenKichCo: e.target.value
                        })
                    }
                />
              </Form.Group>
              <Form.Group className="mb-3" controlId="formTrangThai">
                <Form.Label>Trạng thái</Form.Label>
                <Form.Select
                    value={newSize.trangThai}
                    onChange={(e) =>
                        setNewSize({
                          ...newSize,
                          trangThai: Number(e.target.value)
                        })
                    }
                >
                  <option value={1}>Hiển thị</option>
                  <option value={0}>Ẩn</option>
                </Form.Select>
              </Form.Group>
            </Modal.Body>
            <Modal.Footer>
              <Button variant="secondary" onClick={() => setShowAddModal(false)}>
                Hủy
              </Button>
              <Button variant="primary" type="submit">
                Thêm
              </Button>
            </Modal.Footer>
          </Form>
        </Modal>

        {/* Modal Chỉnh sửa */}
        <Modal show={showEditModal} onHide={() => setShowEditModal(false)} centered>
          <Modal.Header closeButton>
            <Modal.Title>Sửa kích thước</Modal.Title>
          </Modal.Header>
          <Form onSubmit={handleUpdate}>
            <Modal.Body>
              <Form.Group className="mb-3" controlId="formMaEdit">
                <Form.Label>Mã kích thước</Form.Label>
                <Form.Control
                    required
                    type="text"
                    value={editingSize?.ma || ''}
                    onChange={(e) =>
                        setEditingSize({
                          ...editingSize,
                          ma: e.target.value
                        })
                    }
                />
              </Form.Group>
              <Form.Group className="mb-3" controlId="formTenKichCoEdit">
                <Form.Label>Tên kích thước</Form.Label>
                <Form.Control
                    required
                    type="text"
                    value={editingSize?.tenKichCo || ''}
                    onChange={(e) =>
                        setEditingSize({
                          ...editingSize,
                          tenKichCo: e.target.value
                        })
                    }
                />
              </Form.Group>
              <Form.Group className="mb-3" controlId="formTrangThaiEdit">
                <Form.Label>Trạng thái</Form.Label>
                <Form.Select
                    value={editingSize?.trangThai ?? 1}
                    onChange={(e) =>
                        setEditingSize({
                          ...editingSize,
                          trangThai: Number(e.target.value)
                        })
                    }
                >
                  <option value={1}>Hiển thị</option>
                  <option value={0}>Ẩn</option>
                </Form.Select>
              </Form.Group>
            </Modal.Body>
            <Modal.Footer>
              <Button variant="secondary" onClick={() => setShowEditModal(false)}>
                Hủy
              </Button>
              <Button variant="primary" type="submit">
                Lưu
              </Button>
            </Modal.Footer>
          </Form>
        </Modal>
      </div>
  );
};

export default SizeTable;